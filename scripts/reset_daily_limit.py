...
def reset_daily_trade_count():
    ...
    try:
        ...
        print("✅ Daily trade counter reset to 0")
        print()
    ...
    try:
        ...
        if recent_trades:
            ...
            for trade in recent_trades:
                time = trade['timestamp'].split(
                )[1] if ' ' in trade['timestamp'] else trade['timestamp']
                print(
                    f"   {time} - {trade['side']} {trade['symbol']} x {trade['quantity']} @ ${trade['price']}")
            print()
