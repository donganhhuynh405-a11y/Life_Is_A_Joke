...
def generate_report(report_type='weekly'):
    ...
    if script_path.exists():
        ...
        try:
            ...
            cursor.execute("SELECT COUNT(*), SUM(pnl) FROM trades WHERE timestamp > ?", (cutoff_timestamp,))
            trade_count, total_pnl = cursor.fetchone()
            total_pnl = total_pnl or 0
            
            print(f"\n{Colors.BOLD}{report_type.title()} Report:{Colors.NC}")
            print(f"  Period: {cutoff_date.strftime('%Y-%m-%d')} to {datetime.now().strftime('%Y-%m-%d')}")
...
def check_configuration():
    ...
    for name, path in config_files.items():
        if path.exists():
            ...
            print(f"  {Colors.GREEN}✓{Colors.NC} {name:20} ({size} bytes, modified {mtime.strftime('%Y-%m-%d %H:%M')})")
        else:
...
Examples:
  ...
  %(prog)s --monthly-report    # Generate monthly report
    ...
    parser.add_argument('--diagnose-positions', action='store_true', help='Diagnose current positions')
