...
def find_large_directories(base_path: str = '/', min_size_mb: int = 100, max_depth: int = 3) -> List[Tuple[str, int]]:
...
def analyze_disk_usage(base_path: str = '/opt/trading-bot', min_dir_size: int = 100, min_file_size: int = 50):
    ...
    if cache_files:
        recommendations.append("🗑️  Cache: find / -type d -name __pycache__ -exec rm -rf {} + 2>/dev/null")
...
def main():
    ...
    parser.add_argument('--path', default='/opt/trading-bot', help='Base path to scan (default: /opt/trading-bot)')
    parser.add_argument('--min-dir-size', type=int, default=100, help='Minimum directory size in MB (default: 100)')
    parser.add_argument('--min-file-size', type=int, default=50, help='Minimum file size in MB (default: 50)')
