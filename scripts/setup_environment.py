...
def setup_binance_config():
    ...
    config['BINANCE_TESTNET'] = "true" if get_yes_no(
        "Use Binance Testnet?", default=False) else "false"
...
def setup_trading_config():
    ...
    config['TRADING_ENABLED'] = "true" if get_yes_no(
        "Enable live trading?", default=False) else "false"
    config['DEFAULT_SYMBOL'] = get_input(
        "Default trading symbol",
        default="BTCUSDT",
        required=False)
    config['MAX_POSITION_SIZE'] = str(
        get_number(
            "Maximum position size",
            default=0.1,
            min_val=0.001))
    config['STOP_LOSS_PERCENTAGE'] = str(
        get_number(
            "Stop loss percentage",
            default=2.0,
            min_val=0.1,
            max_val=50))
    config['TAKE_PROFIT_PERCENTAGE'] = str(
        get_number(
...
def setup_risk_management():
    ...
    config['MAX_OPEN_POSITIONS'] = str(
        int(get_number("Maximum open positions", default=3, min_val=1)))
    config['MAX_DAILY_LOSS_PERCENTAGE'] = str(
        get_number(
            "Maximum daily loss percentage",
            default=5.0,
            min_val=0.1,
            max_val=100))
    config['POSITION_SIZE_PERCENTAGE'] = str(
        get_number(
...
def setup_notifications():
    ...
    config['ENABLE_NOTIFICATIONS'] = "true" if get_yes_no(
        "Enable notifications?", default=False) else "false"
