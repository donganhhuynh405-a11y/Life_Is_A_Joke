            ...
            if rows:
                ...
                for row in rows:
                    ...
                    if target_db == 'postgresql':
                        ...
                        f.write(f'INSERT INTO "{table}" ({quoted_cols}) VALUES ({", ".join(values)});\n')
                    else:
                        quoted_cols = ', '.join([f'`{c}`' for c in col_names])
                        f.write(f'INSERT INTO `{table}` ({quoted_cols}) VALUES ({", ".join(values)});\n')
...
def main():
    ...
    parser.add_argument('--format', choices=['json', 'sql', 'csv'], default='json', help='Export format')
    parser.add_argument('--compress', action='store_true', help='Compress JSON output (gzip)')
    parser.add_argument('--target', choices=['postgresql', 'mysql'], default='postgresql', help='Target DB for SQL export')
