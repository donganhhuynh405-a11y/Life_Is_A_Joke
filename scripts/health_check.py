    ...
    def check_system_resources(self):
        ...
        try:
            ...
            if memory_percent < 80:
                self.check_pass("Memory Usage", f"{memory_percent}% ({memory_available_gb:.2f}GB available)")
            elif memory_percent < 90:
                self.check_warn("Memory Usage", f"{memory_percent}% ({memory_available_gb:.2f}GB available)")
            else:
                self.check_fail("Memory Usage", f"{memory_percent}% ({memory_available_gb:.2f}GB available)")
        except Exception as e:
        ...
        if os.path.isdir(log_dir):
            ...
            if log_files:
                ...
                if log_files:
                    ...
                    if age_hours < 24:
                        self.check_pass("Recent Activity", f"Last log update: {age_hours:.1f} hours ago")
                    else:
                        self.check_warn("Recent Activity", f"Last log update: {age_hours:.1f} hours ago")
            else:
    ...
    def check_backups(self):
        ...
        if os.path.isdir(backup_dir):
            ...
            if backup_files:
                ...
                if age_hours < 24:
                    self.check_pass("Latest Backup", f"{most_recent.name} ({age_hours:.1f} hours old)")
                elif age_hours < 72:
                    self.check_warn("Latest Backup", f"{most_recent.name} ({age_hours:.1f} hours old)")
                else:
                    self.check_fail("Latest Backup", f"{most_recent.name} ({age_hours:.1f} hours old)")
                
                total_size = sum(f.stat().st_size for f in backup_files) / (1024**3)
                self.check_pass("Backup Files", f"{len(backup_files)} file(s), {total_size:.2f}GB total")
            else:
