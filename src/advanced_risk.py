...
class TradingEnvironment:
    ...
    def _calculate_reward(self, new_value, transaction_costs):
        ...
        old_value = self.portfolio_value_history[-2] if len(
            self.portfolio_value_history) > 1 else self.initial_balance
        ...
        current_drawdown = (self.max_portfolio_value - new_value) / \
...
class RLPortfolioManager:
    ...
    def compute_leverage(self, volatility: float) -> float:
        ...
        leverage = self.leverage_range[0] + (1 - atr_normalized) * \
            (self.leverage_range[1] - self.leverage_range[0])
