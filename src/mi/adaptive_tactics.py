...
class AdaptiveTacticsManager:
    ...
    def _adjust_position_sizing(self, perf_7d: Dict, advanced: Dict) -> List[str]:
        ...
        try:
            ...
            if win_rate > 70 and profit_factor > 2.0 and sharpe > 1.5:
                new_multiplier = 1.3
                adjustments.append("📈 Increasing position sizes by 30% - Excellent performance detected")
                ...
                new_multiplier = 0.5
                adjustments.append("📉 Reducing position sizes by 50% - Poor performance, protecting capital")
    ...
    def _adjust_confidence_threshold(self, perf_7d: Dict, perf_30d: Dict) -> List[str]:
        ...
        try:
            ...
            if win_rate_7d < 40:
                new_threshold = 85.0
                adjustments.append("🎯 Raising confidence threshold to 85% - Only taking highest conviction trades")
            
            # Weak win rate - higher selectivity
            elif win_rate_7d < 50:
                new_threshold = 70.0
                adjustments.append("🎯 Raising confidence threshold to 70% - Increasing trade selectivity")
            
            # Excellent win rate - can be less selective
            elif win_rate_7d > 75 and win_rate_30d > 70:
                new_threshold = 40.0
                adjustments.append("🎯 Lowering confidence threshold to 40% - High win rate allows more opportunities")
    ...
    def _adjust_max_positions(self, advanced: Dict) -> List[str]:
        ...
        try:
            ...
            if max_dd > 25 or sharpe < -1:
                ...
                adjustments.append(f"⚠️ Reducing max positions to {new_max} - High risk detected (DD: {max_dd:.1f}%)")
            elif max_dd > 15 or sharpe < 0:
                new_max = max(1, current_max - 1)
                adjustments.append(f"⚠️ Reducing max positions to {new_max} - Elevated risk (DD: {max_dd:.1f}%)")
            elif max_dd < 5 and sharpe > 2.0:
                new_max = min(10, current_max + 1)
                adjustments.append(f"✅ Increasing max positions to {new_max} - Excellent risk management (DD: {max_dd:.1f}%, Sharpe: {sharpe:.2f})")
