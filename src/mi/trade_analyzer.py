...
class TradeAnalyzer:
    ...
    def analyze_performance(self, days: Optional[int] = None) -> Dict:
        ...
        return {
            ...
            'profit_factor': abs(sum(p['pnl'] for p in profitable) / sum(p['pnl'] for p in losing)) if losing and sum(p['pnl'] for p in losing) != 0 else float('inf')
        }
    ...
    def find_common_patterns_in_profitable_trades(self, min_pnl: float = 0) -> Dict:
        ...
        return {
            ...
            'avg_profit_long': sum(p['pnl'] for p in long_trades) / len(long_trades) if long_trades else 0,
            'avg_profit_short': sum(p['pnl'] for p in short_trades) / len(short_trades) if short_trades else 0
        }
    ...
    def get_recommendations(self) -> List[str]:
        ...
        if overall['win_rate'] < 50:
            recommendations.append(f"⚠️ Win rate is {overall['win_rate']:.1f}%. Consider adjusting strategy parameters.")
        ...
        if overall['profit_factor'] < 1.5:
            recommendations.append(f"⚠️ Profit factor is {overall['profit_factor']:.2f}. Aim for >1.5 for sustainable profitability.")
        ...
        if symbol_stats:
            ...
            recommendations.append(f"📈 Best performing pair: {best_symbol[0]} (P&L: ${best_symbol[1]['total_pnl']:.2f})")
            if worst_symbol[1]['total_pnl'] < 0:
                recommendations.append(f"📉 Worst performing pair: {worst_symbol[0]} (P&L: ${worst_symbol[1]['total_pnl']:.2f}) - Consider disabling or adjusting strategy")
        
        # Average win vs loss
        if abs(overall['avg_loss']) > overall['avg_profit']:
            recommendations.append(f"⚠️ Average loss (${abs(overall['avg_loss']):.2f}) exceeds average profit (${overall['avg_profit']:.2f}). Improve risk/reward ratio.")
