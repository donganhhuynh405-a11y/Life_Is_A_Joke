...
class MLTrainingPipeline:
    ...
    async def train_symbol(self, symbol: str) -> Optional[ModelMetrics]:
        ...
        if existing_model and not self.force_retrain:
            ...
            logger.info(f"✅ Model already exists for {symbol} (accuracy: {metrics.get('accuracy', 'N/A')})")
            logger.info(f"   Skipping training (use force_retrain=True to retrain)")
            return None
        ...
        if self.training_stats['successful'] > 0:
            ...
            for symbol, result in self.training_stats['results'].items():
                if result['status'] == 'success':
                    ...
                    logger.info(f"   • {symbol}: accuracy={metrics['accuracy']:.4f}, f1={metrics['f1_score']:.4f}")
