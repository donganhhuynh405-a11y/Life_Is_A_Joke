...
class AdvancedFeatureEngineer:
    ...
    def volatility_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        features['vol_regime'] = pd.qcut(vol.rank(method='first'), q=5, labels=False, duplicates='drop')
    ...
    def momentum_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        features['awesome_oscillator'] = median_price.rolling(5).mean() - median_price.rolling(34).mean()
    ...
    def pattern_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        features['consecutive_bullish'] = is_bullish.groupby((is_bullish != is_bullish.shift()).cumsum()).cumsum()
        features['consecutive_bearish'] = is_bearish.groupby((is_bearish != is_bearish.shift()).cumsum()).cumsum()
