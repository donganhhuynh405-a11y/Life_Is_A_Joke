...
class HistoricalDataFetcher:
    ...
    async def fetch_full_history(
            ...
            try:
                ...
                if metadata_path.exists():
                    ...
                    if datetime.now() - last_update < timedelta(hours=24):
                        ...
                        logger.info(f"✅ Loaded {len(df)} candles from cache (from {df.index[0]} to {df.index[-1]})")
                        return df
        ...
        logger.info(f"📊 Expected ~{expected_candles:,} candles from {start_date.date()} to {end_date.date()}")
        ...
        while current_start < end_date:
            try:
                ...
                current_start = datetime.fromtimestamp(last_timestamp) + timedelta(minutes=timeframe_minutes)
    ...
    async def update_cached_data(self, symbol: str, timeframe: str = '1h') -> Optional[pd.DataFrame]:
        """
