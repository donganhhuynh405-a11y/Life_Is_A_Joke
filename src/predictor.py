...
class HybridPredictor:
    ...
    async def analyze_symbol(self, symbol: str, data: Dict[str, pd.DataFrame]) -> Optional[TradingSignal]:
        """
        ...
        try:
            ...
            if 'atr_percent' in signal.indicators:
                ...
                if signal.indicators['atr_percent'] < min_volatility:
                    logger.debug(f"Volatility {signal.indicators['atr_percent']}% below minimum {min_volatility}%")
                    return False
            
            # Trend strength filter
            if 'ema20' in signal.indicators and 'ema50' in signal.indicators:
                trend_strength = abs(signal.indicators['ema20'] - signal.indicators['ema50']) / signal.price
    ...
    async def analyze(self, market_data: Dict[str, pd.DataFrame]) -> List[Dict]:
        ...
        logger.info(f"Generated {len([s for s in signals if s['signal'] != 'HOLD'])} trading signals")
        return signals
    ...
    def calculate_risk_adjusted_position(self, signal: Dict, balance: float) -> Dict:
        ...
        """
        strategy_config = self.config.trading.strategies.classic_macd_rsi
